...
class SaveChartDialog(val project: Project, private val chartView: AbstractChartView) : DialogWrapper(project, false) {
    ...
    fun saveChart() {
        ...
        if (wrapper != null) {
            val newSettings = chartView.settings.clone() as Settings
            newSettings.plotSize = Dimension(form.width, form.height)

            val chart = chartView.createChart(newSettings)
            val image = chartView.getImage(chart)
